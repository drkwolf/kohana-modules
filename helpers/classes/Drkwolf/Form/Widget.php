 ...
 * 
 * @author Mikito Takada,drkwolf
 * @package Helpers
 * @version 1.0
 *
 */
class Drkwolf_Form_Widget {
   ...
   * @model : the ORM model
   * @except : list of element to unset
   */
  public function __construct($errors = null, $model= null, $except = null )# {{{
  {
      if ( !empty($errors) ) {
          $this->errors = $errors;
      }
      if( !empty($model) ) {
        if ( is_array($model) ) $defaults = $model;
        elseif( $model instanceof ORM ) $defaults = $model->as_array();
		elseif( is_object($model)) $defaults = (array)$model;
        else throw new InvalidArgumentException('model must must be instance of ORM '.$model);
        ...
        {
          unset($defaults[$except]);
        }

        $this->defaults = $defaults;
      } else {
          $this->defaults = $_POST; //FIXME request->post ?
      }
  ...
  /**
   * this function is used to extract the variable path from the input name in
   * the case the where the name is an array example mode[opt1][opt2] the path
   * mode.opt1.opt2
   * 
   * @param   String $name : name of the variable in the array
   * @return  String array path for function array.
   *
  ...
  private function get_path($name)
  {
	...
	/**
	 * Load values for errors, defaults and values from instance.
	 * @param <type> $name
	 * @param <type> $value
	...
	{
		$attributes = self::add_class($attributes, 'text');
	...
	{
		$attributes = self::add_class($attributes, 'password');
		$this->load_values($name, $value, $attributes);
		return '<li>'
	...
	/**
	 * Bool Checkbox : for bealean fields
	 */
	 public function bcheckbox($name, $checked = FALSE, array $attributes = NULL)
	{
		$this->load_values($name, $value, $attributes);
		return '<li>'
			. Kohana_Form::checkbox($name, $name, $value, $attributes)
			. $this->addAlertSpan((isset($this->errors[$name])?$this->errors[$name]:NULL), $attributes)
			. '</li>';
	}
	...
	{
    return '<li>'
      . Kohana_Form::submit($name, $value, self::add_class($attributes, 'submit'))
      . '</li>';
	}
